...
function getUser() {
  return fetch('https://6639d73239137d0001749e04.caiyicloud.com/cyy_gatewayapi/user/buyer/v3/user_audiences?length=500&offset=0&src=WEB&ver=4.10.1', {
    headers: {
      ...headers
    }
  }).then(res => res.json())
...
async function run() {
  if (user.length >= 2) {
    ...
    grabInfo[0].audiencesList.map(async (item) => {
      await addName(item.name, item.id)
      await sleep(1)
    })
  }
